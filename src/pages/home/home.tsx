...
const Home: React.FC<{ rulingList: IRuling[]; fetchVote: (operation: OPERATION, ruling: IRuling) => void}> = ({ rulingList, fetchVote }) => {
  const processVote = (operation: OPERATION) => fetchVote(operation, rulingList[0]);
            ...
            <div className="featured-card__content">
              ...
              <div className="featured-card__buttons">
                <button
                  ...
                  className="icon-button home items-center"
                  aria-label="thumbs up"
                  onClick={() => processVote(OPERATION.ADD)}
                >
                  <Image
                    src="/thumbs-up.svg"
                    alt="thumbs up"
                    ...
                    height={"0"}
                  />
                  {rulingList?.[0] && <div className="pl-1">{rulingList[0].percent?.positive}%</div>}
                </button>
                <button
                  data-testid="downButton"
                  className="icon-button home items-center"
                  aria-label="thumbs down"
                  onClick={() => processVote(OPERATION.SUBS)}
                >
                  <Image
                    src="/thumbs-down.svg"
                    alt="thumbs down"
                    ...
                    height={"0"}
                  />
                  {rulingList?.[0] && <div>{rulingList[0].percent?.negative}%</div>}
                </button>
              </div>
            </div>
          </div>
